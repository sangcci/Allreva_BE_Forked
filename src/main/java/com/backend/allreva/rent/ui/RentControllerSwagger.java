...
public interface RentControllerSwagger {
    ...
    @Operation(summary = "첫 화면 rent API 입니다.", description = "첫 화면 rent API 입니다. 현재 날짜에서 가장 가까운 콘서트 순으로 5개 정렬")
    Response<List<RentSummaryResponse>> getRentMainSummaries();

    @Operation(summary = "차량 대절 폼 리스트 조회 API", description = """
    ...
    @Operation(summary = "내가 등록한 차 대절 리스트 조회 API", description = """
    ...
    @Operation(summary = "차량 대절 폼 상세 조회 API", description = """
            차량 대절 폼을 상세 조회합니다.
    ...
    @Operation(summary = "입금 계좌 조회 API", description = """
            입금 계좌를 조회합니다.
    ...
    @Operation(summary = "내가 등록한 차 대절 상세 조회 API", description = """
