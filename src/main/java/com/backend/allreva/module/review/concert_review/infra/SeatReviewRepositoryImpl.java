...
public class SeatReviewRepositoryImpl implements SeatReviewRepository {
    ...
    public List<SeatReviewResponse> findReviewsWithNoOffset(
            final SeatReviewSearchCondition condition, final Long currentMemberId) {
        return queryFactory
                .select(Projections.constructor(
                        SeatReviewResponse.class,
                        seatReview.id,
                        seatReview.seat,
                        seatReview.content,
                        seatReview.star,
                        ...
                        list(JPAExpressions.select(seatReviewImage.url)
                ...
                .from(seatReview)
                .join(member)
                .on(seatReview.memberId.eq(member.id))
                .where(
                        createPaginationCondition(condition.lastId(), condition.lastCreatedAt(), condition.sortType()),
                        seatReview.hallId.eq(condition.hallId()))
                .orderBy(getOrderSpecifier(condition.sortType()), seatReview.id.desc())
                .limit(condition.size())
                .fetch();
    }

    private BooleanExpression createPaginationCondition(
            final Long lastId, final LocalDateTime lastCreatedAt, final SortType sortType) {
    ...
    private OrderSpecifier<?> getOrderSpecifier(final SortType sortType) {
        return sortType == SortType.CREATED_ASC ? seatReview.createdAt.asc() : seatReview.createdAt.desc();
    }
}
