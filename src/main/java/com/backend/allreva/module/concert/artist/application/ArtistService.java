...
public class ArtistService {
    ...
    public void saveIfNotExist(final List<ArtistCreateRequest> artists) {
        List<String> ids = artists.stream().map(ArtistCreateRequest::artistId).toList();

        List<Artist> existingEntities = artistRepository.findAllById(ids);

        Set<String> existingIds = existingEntities.stream().map(Artist::getId).collect(Collectors.toSet());

        List<Artist> newArtists = artists.stream()
                .filter(artist -> !existingIds.contains(artist.artistId()))
                .map(req -> Artist.builder().id(req.artistId()).name(req.name()).build())
                .toList();
    ...
    public Artist getArtistById(final String id) {
        return artistRepository.findById(id).orElseThrow(() -> new CustomException(ArtistErrorCode.ARTIST_NOT_FOUND));
    }
}
