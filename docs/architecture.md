...
- profile 설정은 각 모듈의 resources root에 `{module}.yml` 하나로 두고 `spring.config.activate.on-profile`로 profile별 차이를 분리한다.
- dev profile은 로컬 실행용 상수를 직접 둔다. 공개 저장소에 올리면 안 되는 key와 stag/prod profile 값은 환경변수 placeholder로 둔다. 배포에서는 단일 `secret.yml`을 `/app/config/secret.yml`에 마운트하고 `SPRING_CONFIG_IMPORT=optional:file:/app/config/secret.yml`로 주입해 덮어쓴다.
